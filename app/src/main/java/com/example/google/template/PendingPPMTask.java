...
public class PendingPPMTask extends Fragment {
    ...
    String Scan_Type = " ",User_Id,Task_Id,Form_Id,updatedStatus, Site_Location_Id,activity_frequency,task_date,task_end_date,task_status,asset_activity_linking_id,timestartson,Activity_Name,Asset_Code,Asset_Name,Asset_Location,Asset_Type,Status,Assigned_To_User_Group_Id,Group_Name;
            ...
            if (cursor.moveToFirst()){
                do {
                    ...
                    if (calenderCurrent.getTime().after(dateTimeEnd)) {
                        myDb.updatedPPMTaskDetails(Task_Id, "Missed", "Pending", formatDate(calenderCurrent.getTime()), "", User_Id,Assigned_To_User_Group_Id, "");
                    }else {
                        try {
                            if (dateTimeStart.before(LimitTime)||dateTimeStart.equals(LimitTime)){
                                ppmTaskProvider = new PpmTaskProvider(Task_Id,Site_Location_Id,activity_frequency,formatDate(dateTimeStart),task_status,asset_activity_linking_id,formatDate(dateTimeEnd),Activity_Name,Form_Id,Asset_Code,Asset_Name,Asset_Location,Asset_Type,Status,Assigned_To_User_Group_Id,Group_Name,updatedStatus);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (ScanValue == 1){
            ...
            if (result.getContents()!=null){
                if (myDb.getPPMTaskCount(Asset_Code).equals("Pending")){
